...
```{r}
#| results: asis
extract_eq(fit_rice_0, wrap = TRUE, terms_per_line = 3 , use_coefs = TRUE)
```

::: fragment
This first regression is in `level-level` form (without any linear transformation). Thus, interpretations are straightforward: 1 unit change in $x$ causes $\hat \beta$ unit change in $y$
:::
:::
...
mod_notes <- c(
  "Notes: p-values are '+' p < 0.1, '\\*' p < 0.05, '\\*\\*' p < 0.01, '\\*\\*\\*' p < 0.001. Standard errors are reported in parentheses")
lin_prod <- 
  modelsummary(
...
-   Please note, that $\log(\cdot)$ stands for a natural logarithm.
