...
-   **Univariate data analysis**: descriptive analysis of a single variable.

-   What statistical measurements are used to describe a variable?
...
-   Most commonly measures are:
    ...
    -   Median; Mode; Arithmetic Mean; Weighted Mean;
...
congo_food_dta |> filter(Variable == "ENERGY_kcal") |> 
  ggplot() + 
  aes(x = val, fill = Variable ) + 
  geom_histogram(colour = "black") +
  facet_wrap(. ~ Variable, scales = "free") + 
  ylab("Frequency") + xlab("Daily consumption") +
  labs(title = "The DR Congo. Individuals' daily energy consumption") + 
  guides(fill = "none")
```
:::
...
## At home: describe the rest of distributions
...
-   Median is the value of the observation in the center
    ...
    -   after 50% of observations and before the remaining 50%.
...
:::: {.column width="50%"}
::: incremental
-   How to interpret the means?

-   How to interpret the medians?

-   Why means are different from medians?

-   What does the **average of SEX** variable means?

-   Why **median of SEX** variable is equal to 1?
:::
::::
::::::
...
condo_food_mean_sum <-
  ...
  mutate(`Mean of` = SEX)
congo_skew_1 <- 
  condo_food_mean |># filter(SEX == "Male") |> 
  ggplot() +
  aes(x = ENERGY_kcal, fill = SEX ) +
  geom_histogram(colour = "black", position = "stack", alpha = .5) +
...
::: {.column width="30%"}
Arithmetic mean is very sensitive to the extreme observations, outliers, and non-symmetric distribution.

Population may contain extreme values, however, such outliers appearing in a small sample, may [**substantially bias our estimates**]{.underline}.
:::
:::::::
...
1.  Document them:
    ...
    -   Describe, where, why and how many extreme observations were found.
    -   Describe why such extreme observation could occur.

2.  Analyze reasons behind extreme observations:
...
3.  Keep, adjust statistical method, and generalization level:
    ...
    -   Use "robust" measures (median/IQR) to describe data

    -   Consider data "[winsorizing](https://en.wikipedia.org/wiki/Winsorizing)"

4.  Remove some [**only in the exceptional cases!**]{.underline}
:::
...
set.seed(12231)
pov_pop_dta %>% 
  filter(!is.na(`Extreme poverty`)) |> sample_n(12) %>% 
...
::::: {.column width="50%"}
:::: fragment
**How to measure the extreme poverty in the world?**
...
-   The arithmetic mean of these two is:
    ...
    -   (1 % + 49 %) / 2 = 25 %;
    -   but, 25% of their total population is 0.25 \* 1.406B = 352M ..

-   Arithmetic mean implies that there is 351.5 M people living in poverty, but the sum of the number of poor people is 14M + 3M = 17M people.
:::

## Average extreme poverty in the world (3)

-   Arithmetic means, **without accounting for the size/weight** is an inappropriate measure of average poverty.

::: incremental
-   We must weight country-specific poverty level by the country's population.

    -   ( 1% \* 1.4B + 49% \* 6.1 M ) / (1.4B + 6.1 M ) = 1.2 %

-   Weighting are essential in:

    -   Analysis of the representative Individuals/Household surveys.

    -   Macro-level data aggregation.
:::

## At home: Country sizes matters
...
## At home: Exam problems on the central tendency {.smaller}
...
-   Summaries one numeric variables on a single plot
...
-   At home, check: [THE BOXPLOT AND ITS PITFALLS](https://www.data-to-viz.com/caveat/boxplot.html)

-   At home, check: [Wiki: the box plot](https://en.wikipedia.org/wiki/Box_plot)
:::
...
:::: {.column width="30%"}
::: incremental
-   Variance; Central tendency; (Suspected) outliers; Min/Max.
:::
::::
::::::
...
inc_dist_2c <- 
  ...
  scale_x_continuous(
    limits = c(50, 100000),
    breaks = 10 ^ c(2:5) %>% c(., 10^ log10(50)) %>% sort(), 
    trans = "log10",
    labels = label_math(10^.x)(c(1.69, 2:5)))
...
::: {.fragment fragment-index="3" .non-incremental}
