...
# Cumulative Distribution: application
...
set.seed(123342)
gg_gamma_box <-
  tibble(x = rgamma(5000, shape = 2, rate = 2)) %>% 
  # filter(x > 0 & x < 3) %>% 
  ggplot() + 
  aes(x = x) + 
  geom_boxplot() + 
...
## Example: Deciles of income, taxes and subsidies (1)
...
## Example (Exam): Robust vs non robust variance {.smaller}
...
## Example (Exam): Match distribution with the box plot
...
-   How the quntiles are constructed?
...
# Univariate data analysis is not enough.
