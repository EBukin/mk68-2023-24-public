...
If every sample is unique, why [**random sample**]{.underline} is so important?
...
# bmi_gg_3 <- bmi_gg_1
bmi_gg_3 <- bmi_gg_0 + 
  geom_jitter(data = bmi_samples2, alpha = 0.05) +
...
::: nonincremental
-   Apply [**appropriate sampling techniques:**]{.underline} stratified and clustered sampling.
...
To learn more, see Sampling by [@Thompson2012]
...
## Population and samples by research question {.smaller}
...
-   Does so by:
    ...
    -   Using **statistical methods** and **probability theory** to [**reject** of **accept**]{.underline} it.
...
::: incremental
1.  **formulate two statistical hypothesis**:
...
::: incremental
2.  use **statistical tests** and the **probability theory** to reject **H0** (or fail to reject H0) and conclude either:
